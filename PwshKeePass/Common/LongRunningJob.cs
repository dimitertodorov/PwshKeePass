...
using PwshKeePass.Common.Properties;
...
namespace PwshKeePass.Common
{
    ...
    /// </summary>
    public abstract class LongRunningJob : Job
    {
        protected LongRunningJob(string command, string name) : base(command, name)
        {
        }
    ...
    public class LongRunningJob<T> : LongRunningJob, ICommandRuntime where T : KeePassCmdlet
    {
        ...
        protected LongRunningJob(T cmdlet, string command, string name) : this(cmdlet, command, name,
            (cmd) => cmd.ExecuteCmdlet())
        {
        }
        ...
        protected LongRunningJob(T cmdlet, string command, string name, Action<T> executeCmdlet) : base(command, name)
        {
        ...
        /// </summary>
        internal ConcurrentQueue<ShouldMethodStreamItem> BlockedActions
        {
            get { return _actions; }
        }
        ...
        public static LongRunningJob<U> Create<U>(U cmdlet, string command, string name) where U : KeePassCmdlet
        {
            if (cmdlet == null)
            {
            ...
            var job = new LongRunningJob<U>(cmdlet, command, name);
            return job;
        }
        ...
        /// <returns>A job tracking the background execution of the cmdlet</returns>
        public static LongRunningJob<U> Create<U>(U cmdlet, string command, string name, Action<U> executor)
            where U : KeePassCmdlet
        {
            if (null == cmdlet)
            {
                throw new ArgumentNullException(nameof(cmdlet));
            ...
            var job = new LongRunningJob<U>(cmdlet, command, name, executor);
            return job;
        }
        ...
        public static U CopyCmdlet<U>(U cmdlet) where U : KeePassCmdlet
        {
            if (cmdlet == null)
            {
                throw new ArgumentNullException(nameof(cmdlet));
            ...
            foreach (var field in returnType.GetFields(BindingFlags.Instance | BindingFlags.Public |
            ...
            while (!IsFailedOrCancelled(JobStateInfo.State) && _actions.TryDequeue(out stream))
            {
                ...
                catch (InvalidOperationException exception)
                {
                    ...
                    WriteError(new ErrorRecord(
                        new InvalidOperationException(GetShouldMethodFailureReaon(_cmdlet, stream), exception),
                        "InvalidBackgroundOperation", ErrorCategory.InvalidOperation, this));
                }
                finally
                {
        ...
        internal string GetShouldMethodFailureReaon(T cmdlet, ShouldMethodStreamItem method)
        {
            ...
            switch (methodType)
            {
                case ShouldMethodType.ShouldProcess:
                    if (boundParameters != null && boundParameters.ContainsKey("Confirm") &&
                        Convert.ToBoolean(boundParameters["Confirm"].ToString()))
                    {
                        result += Resources.ShouldProcessFailConfirm;
                    }
                    else if (boundParameters != null && boundParameters.ContainsKey("WhatIf") &&
                             Convert.ToBoolean(boundParameters["WhatIf"].ToString()))
                    {
        ...
        {
            get { return _runtime.CurrentPSTransaction; }
        }
        ...
        static bool ShouldConfirm(KeePassCmdlet cmdlet)
        {
        ...
        {
            return !(_cmdlet.MyInvocation.BoundParameters.ContainsKey("Confirm") &&
                     Convert.ToBoolean(_cmdlet.MyInvocation.BoundParameters["Confirm"].ToString())) &&
                   !(_cmdlet.MyInvocation.BoundParameters.ContainsKey("WhatIf") &&
                     Convert.ToBoolean(_cmdlet.MyInvocation.BoundParameters["WhatIf"].ToString())) &&
                   !_shouldConfirm;
        }
        ...
        protected void ThrowIfJobFailedOrCancelled()
        {
            lock (_lockObject)
            {
                if (IsFailedOrCancelled(JobStateInfo.State))
                {
                    throw new LongRunningJobCancelledException(
                        "Azure Long running job is stopping, cannot perform any action");
                }
            }
        }
        ...
        public bool ShouldProcess(string verboseDescription, string verboseWarning, string caption)
        {
            ...
            Exception thrownException;
            return InvokeShouldMethodAndWaitForResults(
                cmdlet => cmdlet.ShouldProcess(verboseDescription, verboseWarning, caption),
                ShouldMethodType.ShouldProcess, out thrownException);
        }
            ...
            out ShouldProcessReason shouldProcessReason)
        {
            ...
            Exception thrownException;
            bool result = InvokeShouldMethodAndWaitForResults(
                cmdlet => cmdlet.ShouldProcess(verboseDescription, verboseWarning, caption,
                    out closureShouldProcessReason),
                ShouldMethodType.ShouldProcess, out thrownException);
        ...
        private bool InvokeShouldMethodAndWaitForResults(Func<Cmdlet, bool> shouldMethod, ShouldMethodType methodType,
            out Exception exceptionThrownOnCmdletThread)
        {
            ...
            using (var gotResultEvent = new ManualResetEventSlim(false))
            {
                EventHandler<JobStateEventArgs> stateChangedEventHandler =
                    ...
                    {
                        WriteDebug(string.Format(Resources.TraceHandleLROStateChange,
                            eventArgs?.PreviousJobStateInfo?.State,
                            eventArgs?.JobStateInfo?.State, eventArgs?.PreviousJobStateInfo?.Reason));
                        if (eventArgs?.JobStateInfo?.State != JobState.Blocked &&
                            eventArgs?.PreviousJobStateInfo?.State == JobState.Blocked)
                        {
                        ...
                        if (IsFailedOrCancelled(eventArgs.JobStateInfo.State) ||
                            eventArgs.JobStateInfo.State == JobState.Stopping)
                        {
                            WriteDebug(Resources.TraceHandlerUnblockJob);
                            lock (resultsLock)
                            {
